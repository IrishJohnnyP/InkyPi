...
class BasePlugin:
    ...
    def render_image(self, dimensions, html_file, css_file=None, template_params={}):
        try:
            ...
            # --- SPECTRA 6 HARDWARE DITHERING INJECTION ---
            if image.mode != 'RGB':
                image = image.convert('RGB')
                
            # Route the screenshot through the hardware-specific palette
            dithered_image = self.image_loader._apply_spectra6_dither(image)
            
            # Explicit memory clearance to prevent Pi Zero RAM spikes
            del image
            gc.collect()
